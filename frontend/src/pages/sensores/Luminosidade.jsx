...
export default function Luminosidade() {
  const [dados, setDados]   = useState([]);
  const [erro, setErro]     = useState('');
  const [sensores, setSens] = useState([]);

  useEffect(() => {
    api.get('api/sensores/?sensor=LUMINOSIDADE').then(r => setSens(r.data)).catch(() => {});
    api.get('api/historicos/?sensor__sensor=LUMINOSIDADE')
    ...
    <Layout>
      <div style={{ padding: '2rem' }}>
        ...
        <p style={{ color: 'var(--muted)', fontSize: 13, marginBottom: '1.5rem' }}>
          Histórico de leituras — unidade: lux
        </p>
        ...
        <div style={{
          ...
          <Table
            columns={['ID', 'Sensor', 'Valor (lux)', 'Timestamp']}
            rows={dados}
